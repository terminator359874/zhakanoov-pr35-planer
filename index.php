...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500&family=IBM+Plex+Sans:wght@400;500;600&display=swap"
        rel="stylesheet">
    ...
    <style>
        ...
        * {
        ...
        .invite-wrap input:focus {
            border-color: var(--accent);
        }

        .invite-wrap input::placeholder {
            color: var(--text-dim);
        }
        ...
        .sidebar-section.stats .sidebar-body {
            max-height: 160px;
        }

        .sidebar-section.activity .sidebar-body {
            flex: 1;
        }
...
<body>
    ...
    <div class="topbar">
        ...
        <a href="calendar.php" class="topbar-btn" style="color:var(--accent); border-color:var(--accent);">📅
            Календарь</a>
        <a href="dashboard.php" class="topbar-btn">📊 Дашборд</a>
        <div class="topbar-spacer"></div>
        <?php if ($invitesCount > 0): ?>
            <button class="topbar-btn" style="border-color:var(--yellow); color:var(--yellow);" data-bs-toggle="modal"
        ...
        <button id="undoBtn" class="topbar-btn" onclick="undoLastMove()" style="display:none;" title="Отменить перемещение (Ctrl+Z)">↩ Отменить</button>
    ...
    <?php if (count($projects) === 0): ?>
        <div style="flex:1;display:flex;align-items:center;justify-content:center;color:var(--text-dim);">
            У вас пока нет проектов. <a href="create_project.php" style="color:var(--accent);margin-left:6px;">Создайте
                новый →</a>
        </div>
    <?php else: ?>
        ...
        <div class="main-area">
            ...
            <div class="board-side">
                <?php foreach ($projects as $i => $project):
                    ...
                    $titles = ['new' => 'Новые', 'progress' => 'В процессе', 'done' => 'Завершены', 'deferred' => 'В работе'];
                                        ...
                                        <?php foreach ($status_tasks as $task): ?>
                                            <div class="task-card task-<?= $task['priority'] ?>" data-id="<?= $task['id'] ?>">
                                                ...
                                                <?php if (!empty($task['description'])): ?>
                                                    <div class="task-desc">
                                                        <?= mb_strimwidth(htmlspecialchars($task['description']), 0, 70, '…') ?></div>
                                                <?php endif; ?>
    ...
    <div class="toast-container position-fixed bottom-0 end-0 p-3">
        <div id="saveToast" class="toast align-items-center text-white border-0" style="background:#1a1f2e;"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <?php if (count($invitations) === 0): ?>
                        <div style="text-align:center;color:var(--text-dim);font-size:12px;">У вас нет новых приглашений.
                        </div>
                    <?php else: ?>
                        <div style="display:flex; flex-direction:column; gap:10px;">
                            <?php foreach ($invitations as $inv): ?>
                                <div
                                    style="border:1px solid var(--border); border-radius:4px; padding:10px; display:flex; justify-content:space-between; align-items:center;">
                                    <div>
                                        <strong><?= htmlspecialchars($inv['project_name']) ?></strong><br>
                                        <span style="font-size:11px; color:var(--text-dim);">От:
                                            <?= htmlspecialchars($inv['from_user_name']) ?></span>
                                    </div>
                                    <div style="display:flex; gap:6px;">
                                        <button class="topbar-btn primary"
                                            onclick="handleInvite(<?= $inv['id'] ?>, 'accept')">Принять</button>
                                        <button class="topbar-btn danger"
                                            onclick="handleInvite(<?= $inv['id'] ?>, 'reject')">Отклонить</button>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
    ...
    <script>
        // ── UNDO STACK ──
        const undoStack = []; // { taskId, prevStatus, prevFromCol, cardEl }
            ...
            document.querySelectorAll('.col-body').forEach(col => {
                ...
                new Sortable(col, {
                    ...
                    onEnd: function (evt) {
                        ...
                        const prevStatus = evt.from.getAttribute('data-status');
                        const card = evt.item;
                        ...
                        fetch('update_task_status.php', { method: 'POST', body: fd })
                            ...
                            .then(data => {
                                ...
                                if (data.success) {
                                    // Сохраняем действие в стек для undo
                                    undoStack.push({ taskId, prevStatus, fromCol: evt.from, toCol: evt.to, card });
                                    if (undoStack.length > 20) undoStack.shift();
                                    updateUndoBtn();
                                    toastMsg.innerText = 'Статус обновлён';
                                } else {
        ...
        async function undoLastMove() {
            ...
            const { taskId, prevStatus, fromCol, toCol, card } = undoStack.pop();
            updateUndoBtn();
            try {
                const fd = new FormData();
                fd.append('id', taskId);
                fd.append('status', prevStatus);
                const r = await fetch('update_task_status.php', { method: 'POST', body: fd });
                const data = await r.json();
                if (data.success) {
                    // Перемещаем карточку обратно в DOM
                    fromCol.appendChild(card);
                    const emptyTo = toCol.querySelector('.empty-col');
                    if (!emptyTo && toCol.querySelectorAll('.task-card').length === 0) {
                        const e = document.createElement('div'); e.className = 'empty-col'; e.textContent = '—';
                        toCol.appendChild(e);
                    }
                    const emptyFrom = fromCol.querySelector('.empty-col');
                    if (emptyFrom && fromCol.querySelectorAll('.task-card').length > 0) emptyFrom.remove();
